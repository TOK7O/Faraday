    ...
    : IMonitoringService
{
    ...
    public async Task ProcessRackReadingAsync(int rackId, decimal temperature, decimal measuredWeight)
    {
        ...
        if (rack is not { IsActive: true }) return;
    ...
    public async Task CheckExpirationDatesAsync()
    {
        ...
        {
            if (alert is { RackId: not null, ProductDefinitionId: not null })
            {
                if (activeProductRackPairs.Contains((alert.ProductDefinitionId.Value, alert.RackId.Value))) continue;
