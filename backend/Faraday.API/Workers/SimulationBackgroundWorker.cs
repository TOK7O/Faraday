    ...
    : BackgroundService
{
    ...
    {
        logger.LogInformation("Simulation Background Worker started. Initializing sensors...");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Error in Simulation Worker cycle.");
            }
        ...
        foreach (var rack in racks)
        {
            ...
            if (rack == victimRack)
            {
                ...
                if (failureType == 0)
                {
                    ...
                    logger.LogInformation($"[SIMULATION] Generating TEMP SPIKE for Rack {rack.Code}");
                }
                else
                {
                    ...
                    logger.LogInformation($"[SIMULATION] Generating WEIGHT DROP for Rack {rack.Code}");
                }
            }
            else
            {
