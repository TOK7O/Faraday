...
const ReportsContent = () => {
  ...
  const [itemViolations, setItemViolations] = useState<
    ItemTemperatureViolation[]
  >([]);
            ...
            {rackUtilization.map((rack) => (
              <tr key={rack.rackCode}>
                ...
                <td>
                  ...
                  >
                    <progress
                      value={rack.slotUtilizationPercentage}
                      max="100"
                      style={{ width: "100px" }}
                    ></progress>
                    <span>
                      {rack.slotUtilizationPercentage}% ({rack.occupiedSlots}/
                      {rack.totalSlots})
                    </span>
                  </div>
                </td>
                <td>
                  ...
                  >
                    <progress
                      value={rack.weightUtilizationPercentage}
                      max="100"
                      style={{ width: "100px" }}
                    ></progress>
                    <span>
                      {rack.weightUtilizationPercentage}% (
                      {rack.currentWeightKg.toFixed(1)}
                      {reportsT.common?.kg || "kg"})
                    </span>
                  </div>
                </td>
                <td>
                  {rack.maxWeightKg} {reportsT.common?.kg || "kg"}
                </td>
              </tr>
            ))}
          </tbody>
  ...
  const renderSensorsView = () => (
    <div className="view-container">
      <div className="table-container">
        ...
        <table className="data-table">
          ...
          <tbody>
              ...
              <tr key={v.readingId}>
                ...
                <td>
                  <span className="badge danger">
                    +{v.violationDegrees.toFixed(1)}°C
                  </span>
                </td>
            ...
            {rackViolations.length === 0 && itemViolations.length === 0 && (
              <tr>
                <td colSpan={6} className="empty-state">
                  {reportsT.sensors.emptyViolations}
                </td>
              </tr>
            )}
          </tbody>
      ...
      >
        <div className="table-container">
          <h3>
            <Thermometer size={16} /> {reportsT.sensors.tempHistory}
          </h3>
          <table className="data-table">
            <thead>
              <tr>
                <th>{reportsT.sensors.table.rack}</th>
                <th>{reportsT.inventory.table.temp}</th>
                <th>{reportsT.sensors.table.time}</th>
              </tr>
            </thead>
        ...
        <div className="table-container">
          <h3>
            <Scale size={16} /> {reportsT.sensors.weightHistory}
          </h3>
          <table className="data-table">
            <thead>
              <tr>
                <th>{reportsT.sensors.table.rack}</th>
                <th>{reportsT.sensors.table.measured}</th>
                <th>{reportsT.sensors.table.discrepancy}</th>
                <th>{reportsT.sensors.table.time}</th>
              </tr>
            </thead>
            <tbody>
              {weightHistory.map((w) => (
                <tr key={w.id}>
                  <td>{w.rackCode}</td>
                  <td>
                    {w.measuredWeightKg.toFixed(2)}{" "}
                    {reportsT.common?.kg || "kg"}
                  </td>
                  <td
                    style={{
                      color:
                        Math.abs(w.discrepancyKg) > 0.5 ? "#facc15" : "inherit",
                    }}
                  >
                    {w.discrepancyKg > 0 ? "+" : ""}
                    {w.discrepancyKg.toFixed(2)} {reportsT.common?.kg || "kg"}
                  </td>
    ...
    <div className="view-container">
      <div className="table-container" style={{ marginBottom: "2rem" }}>
        ...
        <table className="data-table">
          <thead>
            <tr>
              <th>{reportsT.alerts.table.severity}</th>
              <th>{reportsT.alerts.table.rack}</th>
              <th>{reportsT.alerts.table.message}</th>
              <th>{reportsT.alerts.table.duration}</th>
            </tr>
          </thead>
          <tbody>
            {activeAlerts.map((a) => (
              <tr key={a.id}>
                ...
                <td>{a.message}</td>
                <td>
                  {a.durationMinutes} {reportsT.common?.mins || "mins"}
                </td>
              </tr>
            ))}
      ...
      <div className="table-container">
        ...
        <table className="data-table">
          <thead>
            <tr>
              <th>{reportsT.alerts.table.status}</th>
              <th>{reportsT.alerts.table.rack}</th>
              <th>{reportsT.alerts.table.message}</th>
              <th>{reportsT.alerts.table.created}</th>
              <th>{reportsT.alerts.table.resolved}</th>
            </tr>
          </thead>
          <tbody>
            {alertHistory.map((a) => (
              <tr key={a.id}>
                <td>
                  <span
                    className={`badge ${a.isResolved ? "success" : "danger"}`}
                  >
                    {a.isResolved
                      ? reportsT.alerts.status.resolved
                      : reportsT.alerts.status.active}
                  </span>
                </td>
                <td>{a.rackCode || "-"}</td>
                <td>{a.message}</td>
        ...
        <div className="stats-grid">
          <div className="stat-card">
            ...
            <div className="sub-value">
              {stats.occupiedSlots} / {stats.totalSlots} {reportsT.stats.slots}
            </div>
          </div>
          <div className="stat-card">
            ...
            <div className="sub-value">
              {reportsT.stats.cap}: {(stats.totalCapacityKg / 1000).toFixed(1)}t
            </div>
          </div>
          <div className="stat-card">
