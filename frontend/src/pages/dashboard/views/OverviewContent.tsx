...
import { Activity, ShieldCheck, Box, MousePointer2, Loader2 } from "lucide-react";
...
const Rack3D = ({ position, rack, items }: { position: [number, number, number], rack: Rack, items: FullInventoryItem[] }) => {
            ...
            {[...Array(rows)].map((_, rIndex) => {
                ...
                return (
                    <group key={`r-${rIndex}`} position={[0, rIndex * shelfHeight, 0]}>
                        ...
                        {rowItems.map((item) => (
                            <mesh key={`item-${item.itemId}`} position={[(item.slotX - 1) * 1.4 + 0.7, 0.4, 0]}>
            ...
            <div className="bento-dashboard">
                <div className="bento-card" style={{ gridColumn: "span 2", gridRow: "span 2", minHeight: "550px", padding: 0, overflow: "hidden", position: "relative" }}>
                    ...
                    <div style={{ position: "absolute", top: "1.8rem", left: "1.8rem", zIndex: 10 }}>
                        ...
                        {isLoading && (
                            <div style={{ marginTop: '8px', display: 'flex', alignItems: 'center', gap: '8px', color: 'rgba(255,255,255,0.5)', fontSize: '0.8rem' }}>
                        ...
                        <Canvas shadows dpr={[1, 2]}>
                            <Suspense fallback={null}>
                                ...
                                <Bounds fit clip observe margin={1.2}>
                                    <group position={[-spacingX / 2, 0, 0]}>
                                        {rackElements}
                                    </group>
                                </Bounds>

                                <gridHelper args={[100, 100, "#27272a", "#111111"]} position={[0, -0.01, 0]} />
                            </Suspense>
                        </Canvas>
                    </div>
                </div>
