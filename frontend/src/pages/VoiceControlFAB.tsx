...
import { Mic, MicOff, Loader2, AlertCircle, CheckCircle2, X, ChevronDown, ChevronUp } from 'lucide-react';
...
export const VoiceControlFAB = () => {
    ...
    useEffect(() => {
        if (!listening && transcript.length > 0 && !isProcessing && !isWaitingForProcessing && !feedback) {
            handleStopAndSend();
        }
    ...
    const summarizeResponseForSpeech = (response: any): string => {
        if (!response.success) {
            return response.message || (lang === 'pl' ? "Wystąpił błąd." : "An error occurred.");
        }

        const { actionPerformed, data } = response;

        // Truncate for speech to be concise, but keep full for UI
        if (actionPerformed === "multi_step_execution" && data?.results?.inventory) {
            const inventory = data.results.inventory;
            if (inventory.length === 0) return lang === 'pl' ? "Brak produktów." : "No products found.";

            const itemsSummary = inventory.slice(0, 2).map((item: any) =>
                `${item.productName || item.name || item.ProductName}: ${item.totalQuantity || item.quantity || item.TotalQuantity}`
            ).join(", ");

            const totalCount = inventory.length;
            const extra = totalCount > 2 ? (lang === 'pl' ? " i inne." : " and more.") : ".";

            return lang === 'pl' ? `Na stanie: ${itemsSummary}${extra}` : `In stock: ${itemsSummary}${extra}`;
        }

        let message = response.message || "";
        const genericPhrases = ["Check response data", "Sprawdź dane"];
        if (!message || genericPhrases.some(p => message.includes(p))) {
            message = lang === 'pl' ? "Zakończono pomyślnie." : "Completed successfully.";
        }
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    setFeedback({ type: 'error', msg: result.message || (lang === 'pl' ? "Niepowodzenie" : "Failure") });
                    speak(spokenSummary);
                }
            } catch (error: any) {
                console.error("[Faraday Voice] API Error:", error);
                const apiErrorMsg = error.response?.data?.message || (lang === 'pl' ? "Błąd serwera." : "Server error.");
                setFeedback({ type: 'error', msg: apiErrorMsg });
                speak(lang === 'pl' ? "Wystąpił błąd." : "An error occurred.");
            } finally {
            ...
            return (
                <ul className="data-list">
                    {data.map((item: any, i: number) => (
                        <li key={i} className={`data-item ${typeof item === 'string' ? 'is-string' : ''}`}>
                            {typeof item === 'string' ? (
                                <span className="full-text">{item}</span>
                            ) : (
                                <>
                                    <span className="name">{item.productName || item.name || item.ProductName || item.Name || Object.values(item)[0]}</span>
                                    <span className="value">
                                        {item.totalQuantity ?? item.quantity ?? item.TotalQuantity ?? item.Quantity ?? (typeof Object.values(item)[1] === 'number' ? Object.values(item)[1] : "")}
                                    </span>
                                </>
                            )}
                        </li>
                    ...
                    {Object.entries(data).map(([key, value]: [string, any], i: number) => {
                        ...
                        return (
                            <li key={i} className="data-item stat">
                                <span className="name">{key.replace(/([A-Z])/g, ' $1').toLowerCase()}</span>
    ...
    return (
        <div id="faraday-voice-assistant" className="voice-assistant-fab">
            {(listening || feedback || isProcessing || isWaitingForProcessing) && (
                ...
                `}>
                    <div className="bubble-header" onClick={responseData ? toggleExpand : undefined}>
                        <div className="bubble-status-icon">
                            {(isProcessing || isWaitingForProcessing) && <Loader2 className="animate-spin" size={16} />}
                        ...
                        <span className="bubble-text">
                            {(isProcessing || isWaitingForProcessing)
                                ...
                                : (feedback ? feedback.msg : (transcript || (lang === 'pl' ? "Słucham..." : "Listening...")))}
                        </span>
                        <div className="bubble-actions">
                            {responseData && (
                                <button onClick={(e) => { e.stopPropagation(); toggleExpand(); }} className="action-btn">
                                    {isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                                </button>
                            )}
                            {feedback && (
                                <button onClick={(e) => { e.stopPropagation(); closeBubble(); }} className="action-btn close">
                                    <X size={14} />
                                </button>
                            )}
                        </div>
            ...
            <button
                ...
                title={lang === 'pl' ? "Asystent Głosowy" : "Voice Assistant"}
            >
                {(isProcessing || isWaitingForProcessing) ? (
