...
export const MoveModal = ({
  ...
  const handleSlotSelection = (slotX: number, slotY: number) => {
    if (!item) return;
    const isOccupied = inventory.some(
      (i) =>
        i.rackCode === targetRackCode && i.slotX === slotX && i.slotY === slotY,
    );
    if (
      item.rackCode === targetRackCode &&
      item.slotX === slotX &&
      item.slotY === slotY
    )
      return;
            ...
            <form onSubmit={handleSubmit} className="ht-form">
              ...
              >
                <p style={{ margin: "0.25rem 0", fontSize: "0.9rem" }}>
                  <strong>{invT.product}:</strong> {item.productName}
                </p>
                <p
                  style={{
                    margin: "0.25rem 0",
                ...
                >
                  <strong>{invT.code}:</strong> {item.barcode}
                </p>
                <p
                  style={{
                    margin: "0.25rem 0",
                ...
                >
                  <strong>{invT.from}:</strong> {item.rackCode} (K-{item.slotX},
                  R-{item.slotY})
                </p>
              </div>
              ...
              {selectedRack && (
                <div style={{ marginTop: "1rem" }}>
                  ...
                  <div className="mini-grid" style={{ pointerEvents: "auto" }}>
                    ...
                    >
                      {Array.from({ length: selectedRack.m }).map(
                        (_, rIndex) => {
                          const rowNum = selectedRack.m - rIndex;
                          return Array.from({ length: selectedRack.n }).map(
                            (_, cIndex) => {
                              const colNum = cIndex + 1;
                              const isOccupied = inventory.some(
                                (i) =>
                                  i.rackCode === targetRackCode &&
                                  i.slotX === colNum &&
                                  i.slotY === rowNum,
                              );
                              const isCurrent =
                              ...
                              const tooTall = product
                                ? product.heightMm > selectedRack.maxHeight
                                : false;
                              const tooWide = product
                                ? product.widthMm > selectedRack.maxWidth
                                : false;
                              const tooDeep = product
                                ? product.depthMm > selectedRack.maxDepth
                                : false;
                              const tooHeavy = item
                                ? item.productWeightKg > selectedRack.maxWeight
                                : false;

                              const tempMin = product?.requiredMinTemp ?? -999;
                              const tempMax = product?.requiredMaxTemp ?? 999;
                              const tempMismatch =
                                tempMin > selectedRack.tempMax ||
                                tempMax < selectedRack.tempMin;
                              ...
                              return (
                                <div
                                  ...
                                  onClick={() =>
                                    ...
                                    !isCurrent &&
                                    !isIncompatible &&
                                    handleSlotSelection(colNum, rowNum)
                                  }
                                  style={{
                                    aspectRatio: "1",
                                    ...
                                    borderRadius: "4px",
                                    cursor:
                                      isOccupied || isCurrent || isIncompatible
                                        ? "not-allowed"
                                        : "pointer",
                                    border: isTarget
                                      ? "2px solid white"
                                    ...
                                    opacity:
                                      isCurrent || isIncompatible ? 0.4 : 1,
                                    transition:
                                  ...
                                  title={
                                    isOccupied
                                      ? invT.tooltips.occupied
                                      : isCurrent
                                        ? invT.tooltips.current
                                        : isIncompatible
                                          ? `${invT.tooltips.incompatible}: ${tooHeavy ? invT.tooltips.tooHeavy : tempMismatch ? invT.tooltips.tempMismatch : invT.tooltips.dimMismatch}`
                                          : `${invT.tooltips.select} ${colNum}x${rowNum}`
                                  }
                                >
                                  {isTarget && colNum}
                                  {isIncompatible &&
