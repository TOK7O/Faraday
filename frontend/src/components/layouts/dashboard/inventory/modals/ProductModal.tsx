...
export const ProductModal = ({ open, onOpenChange, onSave, editingProduct, hasInventoryItems = false }: ProductModalProps) => {
    ...
    const validateField = (name: string, value: string, formData: FormData) => {
        let error = "";
        if ((name === "name" || name === "scanCode") && !value.trim()) error = invT.errors.required;
        ...
        if (["weightKg", "widthMm", "heightMm", "depthMm"].includes(name)) {
            ...
            setErrors(prev => ({ ...prev, dimensions: (isNaN(w) || w <= 0 || isNaN(h) || h <= 0 || isNaN(d) || d <= 0) ? invT.errors.allDimsGt0 : "" }));
        }
        ...
        if (name === "requiredMinTemp" || name === "requiredMaxTemp") {
            ...
            setErrors(prev => ({ ...prev, tempRange: (!isNaN(min) && !isNaN(max) && min > max) ? invT.errors.tempRange : "" }));
        }
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className="dialog-content-ht product-modal">
                    ...
                    <div className="modal-header">
                        <Dialog.Title><h2>{editingProduct ? invT.titleEdit : invT.titleNew}</h2></Dialog.Title>
                        <Dialog.Description className="visually-hidden">{invT.description}</Dialog.Description>
                            ...
                            <div className="input-group">
                                ...
                                <div className="file-input-wrapper">
                                    ...
                                    <label className="file-upload-label">
                                        <FileImage size={18} />
                                        <span className="file-name">{fileName || invT.selectFile}</span>
                        ...
                        <div className="input-row">
                            ...
                            <div className="input-group">
                                ...
                                <div className="multi-input-group compact">
                                    <input type="number" name="widthMm" placeholder={invT.width} required className={hasInventoryItems ? "readonly-field" : ""}
                                        defaultValue={editingProduct?.widthMm} readOnly={hasInventoryItems} onChange={handleChange} />
                                    <input type="number" name="heightMm" placeholder={invT.height} required className={hasInventoryItems ? "readonly-field" : ""}
                                        defaultValue={editingProduct?.heightMm} readOnly={hasInventoryItems} onChange={handleChange} />
                                    <input type="number" name="depthMm" placeholder={invT.depth} required className={hasInventoryItems ? "readonly-field" : ""}
