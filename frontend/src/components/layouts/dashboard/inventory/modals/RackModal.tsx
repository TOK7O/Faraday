...
export const RackModal = ({
  ...
  const runValidation = () => {
    ...
    if (!code) {
      ...
      !editingRack &&
      existingRacks.some((r) => r.code.toUpperCase() === code)
    ) {
    ...
    const validatePositive = (
      ...
      if (formData.has(name)) {
        if (isNaN(val)) newErrors[key] = invT.errors.number;
          ...
          newErrors[key] = invT.errors.gt0.replace("{label}", label);
      }
    };
    ...
    if (minT && maxT) {
      ...
      if (minNum > maxNum) {
        newErrors.tempRange = invT.errors.tempRange
          .replace("{min}", minNum.toString())
          .replace("{max}", maxNum.toString());
      }
    }
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content-ht">
          ...
          <div className="modal-header">
            <Dialog.Title>
              <h2>{editingRack ? invT.titleEdit : invT.titleNew}</h2>
            </Dialog.Title>
          ...
          <form className="ht-form" ref={formRef} onSubmit={handleSubmit}>
            ...
            <div className="input-row">
              <div className="input-group">
                ...
                <input
                  ...
                  style={{
                    ...
                    cursor: hasItems ? "not-allowed" : "text",
                  }}
                />
              ...
              <div className="input-group">
                <label>
                  {invT.environment} <ErrorLabel field="tempRange" />
                </label>
                <div
                ...
                >
                  <input
                    ...
                    name="minTemperature"
                    onChange={handleInputChange}
                    defaultValue={editingRack?.tempMin ?? ""}
                    placeholder="Min"
                    required
                    readOnly={hasItems}
                    style={{
                  ...
                  <input
                    ...
                    name="maxTemperature"
                    onChange={handleInputChange}
                    defaultValue={editingRack?.tempMax ?? ""}
                    placeholder="Max"
                    required
                    readOnly={hasItems}
                    style={{
